...
def rulare_neat(epoci_dorite, fitness_dorit, tst, nume):
    ...
    while actual > fitness_dorit:
        ...
        start_time = time.time()
        set_paraleliat = sc.parallelize(copii)
        set_paraleliat = set_paraleliat.map(crossover)
        set_paraleliat = set_paraleliat.collect()
        ...
        for i in range(0, len(spc.spec)):
            for genom in spc.spec[i].genomi:
                x = genom
                lista_genomi += [x]
        genomi_paralelizati = sc.parallelize(lista_genomi)
        genomi_paralelizati = genomi_paralelizati.map(feed_forwoard)
        genomi_paralelizati = genomi_paralelizati.collect()
